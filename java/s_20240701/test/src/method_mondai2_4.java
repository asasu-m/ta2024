...
public class method_mondai2_4
{
    ...
    public static void mati(String cityname)
    {
        System.out.println("ーー"+cityname+"ーー");
        System.out.println(cityname+"についた。一息できそうだ。。");
        System.out.print("どこに行く？(1)教会(2)雑貨屋(3)武器屋");
    ...
    public static void kyokai()
    {
        System.out.println("勇者パーティーはHPが全回復した");
        healHP(10);
        playershow();
        System.out.println("神のお告げが聞こえます");
        kaminootuge();
    }
    public static void kaminootuge()
    {
        for(int i=0;i<PLAYERCNT;i++)
        {
            System.out.println("勇者1:"+playerName[i]+" XP : "+playerXP[i]);
        }
    }
    ...
    public static int check_ALL_DEAD_PLAYER()
    {
        for(int i=0;i<PLAYERCNT;i++)
        {
            if(playerHP[i]>0)
    ...
    public static void enemyAttack()
    {
        ...
        {
            System.out.println(enemyName[i]+"の攻撃");
            Random rnd = new Random();
            int player_select;
            do{
                player_select=rnd.nextInt(PLAYERCNT);
                if(playerHP[player_select]>0)
                {
                    break;
                }
                if(check_ALL_DEAD_PLAYER()==ALL_DEAD)
                {
                    System.out.println("勇者一行は全滅した・・");
                    System.exit(0);
                }
            }while(true);
            System.out.println("勇者："+playerName[player_select]+"に2ダメージ");
            playerHP[player_select]-=2;
